...
<!--          <div id="mse"></div>-->
          <video controls ref="videoRef"
                 :src="thisFile.fileUrl??thisFile.downloadUrl" style="height: 300px;border-radius: 10px;"
          ></video>
        </div>
      </div>
      <div class="upload-info">
        <div class="info-data">
          <div class="info-text">
            <div class="info-text-name">{{processResult===2?  thisFile.filename:thisFile.name }}</div>
            ...
            <div class="info-text-after" v-if="changeType === '7'">
              输出音频格式：
              <span style="font-size: 20px;margin-left: 10px">{{ type }}</span>
            </div>
            <div class="info-text-after" v-else-if="changeType === '10'">
              视频压缩率：
              <span style="font-size: 20px;margin-left: 10px">{{ type }}%</span>
            </div>
            <div class="in fo-text-other" v-else>视频时长：{{ thisFile.duration }}s</div>
            <div v-if="changeType === '7'" class="info-text-other">处理类型：视频压缩</div>
