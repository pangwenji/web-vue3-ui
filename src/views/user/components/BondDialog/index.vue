<template>
    <el-dialog v-model="visibled" width="500px" :before-close="onBeforeClose" title="保证金退款" custom-class="dialog-center">
...
<script>
import { pledgeRefund } from "@/api/pay/pay";
...
const setup = (prop,{emit}) => {
    ...
    const handleSubmit = () => {
        ...
        pledgeRefund({ orderNo }).then(res => {
            loadingSubmit.value = false;
            onBeforeClose();
            store.commit('SET_MARGIN',0)
            emit('onSubmit',refundData.value)
            ElMessage.success('保证金退款成功');
        }).catch(err => {
            loadingSubmit.value = false;
        })
    }
