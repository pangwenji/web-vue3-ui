<template>
	<div v-loading="imgloading" element-loading-background="#0a0b0d" element-loading-text="图片加载中..." class="image-content">
		...
		>
			<div :style="{width:imgBoxWidth+'px', height:imgBoxHeight+'px',left:imgBoxLeft+'px',top:imgBoxTop+'px'}" style="position:absolute;transform:none;" ref="jieTu">
				<img style="width:100%;height:100%;" :src="fileDetail.sourceUrl" alt="">
				<slot></slot>
			</div>
		</div>
		<div style="width:100%;height:25px;background-color: #20222B;"></div>
		<div class="img-control" :style="{position:!isFullScreen?'fixed':'',bottom:!isFullScreen?'0':'','background-color': !isFullScreen?'#20222B':''}">
...
const jieTu = ref('')
function screenshot() {
	html2canvas(jieTu.value, { useCORS: true, logging: true,allowTaint: true }).then((canvas) => {
		console.log('jieTu--', canvas.toDataURL());
	})
}
