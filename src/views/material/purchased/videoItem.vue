...
<!--          <div>{{ secondToDate(list.fileData?.Duration) || '00:00' }}</div>-->
        </div>
        <div class="image-cell" :style="list.parentStyle">
          ...
          <div class="image-content" @click="toDetails(list.material.id)">
            <img :src="list.material.coverUrl" alt="素材图片">
            <!--            <div-->
            <!--                class="file-item-card"-->
            <!--                @dblclick.stop="handleDetail"-->
            <!--                @click.stop="toDetails(list.material.id)"-->
            <!--            >-->
            <!--              <div class="file-item-content file-item-type5">-->
            <!--                <div-->
            <!--                    class="file-item-cover file-item-video"-->
            <!--                    @mousemove.stop="handleMouseMoveVideo($event,list)"-->
            <!--                >-->
            <!--                  <div-->
            <!--                      class="sprite"-->
            <!--                      :style="{-->
            <!--              width: `${list.viewWidth}px`,-->
            <!--              height: `${list.viewHeight}px`,-->
            <!--              'background-image': 'url(' + list.material.fileAttachedInfoDto.spriteImage + ')',-->
            <!--              'background-position-y': '-' + String(list.offsetY) + 'px',-->
            <!--            }"-->
            <!--                  ></div>-->
            <!--                  <div-->
            <!--                      class="cover"-->
            <!--                      :style="{ 'background-image': `url(${list.material.fileAttachedInfoDto.cover})` }"-->
            <!--                  ></div>-->
            <!--                  &lt;!&ndash;          <div class="time">{{ secondToDate(data.fileData.Duration) }}</div>&ndash;&gt;-->
            <!--                  <div class="play">-->
            <!--                    <el-icon>-->
            <!--                      <VideoPlay/>-->
            <!--                    </el-icon>-->
            ...
            <!--                  <div class="progress">-->
            <!--                    <div-->
            <!--                        :style="{ width: String(list.percentage) + '%' }"-->
...
function appendImage(row) {
  ...
  // row.fileData=JSON.parse(row.material.fileAttachedInfoDto.fileInfo.metaData)
  row.style = {}
  row.parentStyle = {}
  let img = new Image();
  img.src = row.material.coverUrl;
  img.onload = function () {
    let width = img.width;
    let height = img.height;
    if (height <= 300) { //设置阈值小于这个值将宽高放大 300px
      height = height + 300;
      width = (width / img.height) * height;
    }
...
const handleMouseMoveVideo = (e, row) => {
  const spriteImageInfo = JSON.parse(row.material.fileAttachedInfoDto.spriteImageInfo || '{}');
