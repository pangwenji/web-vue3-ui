                ...
                <template v-else>
                    <div class="link-container">
                        <div class="link-container-content">
                            <div>请点击链接，审阅{{resultData.info.name}}</div>
                            <div>链接：{{resultData.url}}</div>
                            <div v-if="resultData.info.password">密码：{{resultData.info.password}}</div>
                        </div>
                        <el-button type="primary" class="copy-btn" @click="handleCopyText(resultData.url)">复制</el-button>
                    </div>
...
<script>
import { getShareDetails } from '@/api/project/files.js'
...
const setup = (prop,{emit})=>{
    ...
    const handleCopyText = async (text)=>{
        try{
            const context = `请点击链接，审阅 ${resultData.value.info.name}
链接: ${resultData.value.url}${resultData.value.info.password?'\n密码: '+ resultData.value.info.password:''}
`
