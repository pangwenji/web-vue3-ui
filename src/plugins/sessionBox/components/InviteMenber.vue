...
<div class="create-team-box">
    <el-dialog v-model="visibled" :close-on-click-modal="false" width="530px" title="邀请联系人" :before-close="onBeforClose" custom-class="dialog-center create-team-dialog">
                    ...
                    <div class="group-user-item" v-for="(item,index) in searchData" :key="index">
                        ...
                        <template v-if="item.msgImContacts&&item.msgImContacts.length&&isOpen(item)">
                            <div v-for="(option,code) in item.msgImContacts" :key="code"  class="group-user-info">
                                ...
                                <div class="check-box" :class="{check:checkUser(option),disabled: option.disabled}" @click="handleCheckUser(option)">
        ...
        <div style="text-align:center; margin-top: 20px">
            ...
            <el-button type="primary" style="width: 106px" class="is-custom" :disabled="!selectUser.length" :loading="loadingSubmit" @click="handleSubmit" >邀请加入</el-button>
        </div>
    </el-dialog>
</div>
</template>
...
const setup = (prop,{emit})=>{
    ...
    const handleInputSearch = (e)=>{
        ...
        if(e){
            ...
            personlGroup.value.forEach(item=>{
                item.msgImContacts.forEach(option=>{
                    option.disabled = menberAccounts.value.indexOf(option.contactImAccountId) > -1;
                })
            })
    ...
    const handleSubmit = ()=>{
        ...
        store.dispatch('IM/handleInviteMember',{teamId,accounts,receiveUsers}).then(res=>{
    ...
    const open = (team)=>{
        ...
        loadingState.value = true;
         store.dispatch('IM/handleGeTeamMenber',team).then(menbers=>{
            store.dispatch('IM/handleGetGroup').then(res=>{
                loadingState.value = false;
                teamData.value = team;
                menberAccounts.value = menbers.map(item=>item.account);
                handleInputSearch();
            }).catch(err=>{
                onBeforClose();
                loadingState.value = false;
